function parseSquare(str) {
  return (
    str.charCodeAt(0) -
    "a".charCodeAt(0) +
    8 * (7 - str.charCodeAt(1) + "1".charCodeAt(0))
  );
}
...
function Piece(n, c, s) {
  ...
  this.xfen = function () {
    var glyph =
      this.name.toLowerCase() in __fairyHelper.override
...
function Board() {
  ...
  this.shift = function (a, b) {
    ...
    pb = to_xy(b);
    this.transform(function (x, y) {
      return from_xy(x + pb.x - pa.x, y + pb.y - pa.y);
    });
  };
  ...
  this.mirror = function (a, b) {
    if (a == 56) {
      ...
      if (b == 63)
        ...
        // --> a8
        this.transform(function (x, y) {
          return from_xy(x, 7 - y);
        });
    } else if (a == 63 && b == 0) {
      // h1 <--> a8
      this.transform(function (x, y) {
        return from_xy(7 - y, 7 - x);
      });
    }
  };
  ...
  this.fromPiecesClause = function (p) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      while (
        j < words.length &&
        (matches = words[j].match(/([0-9a-z][0-9a-z]?)([a-h][1-8])+/))
      ) {
  ...
  this.fromAlgebraic = function (algebraic) {
    for (color in algebraic) {
      ...
      for (var i = 0; i < algebraic[color].length; i++) {
        ...
        if (
          (matches = words[words.length - 1]
            .toLowerCase()
            .match(/([a-z][0-9a-z]?)([a-h][1-8])+/))
        ) {
  ...
  this.toHtml = function () {
    ...
    for (var i = 0; i < 8; i++) {
      for (var j = 0; j < 8; j++) {
        var xfen =
          this.board[i * 8 + j] == null ? "" : this.board[i * 8 + j].xfen();
        retval +=
          '<a class="cp' + xfen2spritedecl(xfen, (i + j) % 2) + '"></a>';
      }
    }
    return retval;
  };
...
function xfen2spritedecl(xfen, color) {
  if ("" == xfen) return ["EQ", "FQ"][color];

  sprite = {};
  matches = xfen.match(/^\(?(B?)(!?)([kqrbsnpeaofwdx])([1-7])?\)?$/i);
  if (matches) {
    sprite = {
      glyph: matches[3].toLowerCase(),
      rot: matches[4],
      border: matches[1] != "",
    };
